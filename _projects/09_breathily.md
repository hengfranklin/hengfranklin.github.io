...
Breathily was a startup we built to make lung function assessment more accessible for patients with physical disabilities, including ALS and related neuromuscular conditions. Standard spirometry requires a tight mouth seal and forceful exhalation, which is difficult or impossible for patients with facial muscle weakness or limited mobility. Our system estimates pulmonary function from **chest wall motion** captured by an Intel RealSense depth camera, no mouthpiece required, and reports the full spirometric panel (FVC, FEV1, FEV1/FVC, PEF, FEF25/50/75, FEF25–75) in real time.
...
The chest displacement signal is the raw input to spirometry. The ROI for "chest" is constructed from skeleton keypoints:
...
Raw chest displacement is in meters of depth change. Converting to liters of air uses a per-patient regression model trained on paired Breathily-vs-spirometer recordings:
...
We evaluated Breathily in an IRB-approved clinical study at the UCSF Adult Pulmonary Function Lab with simultaneous depth capture and spirometry.
